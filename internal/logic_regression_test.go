...
func TestGetProviderNormalizesConfiguredProviderName(t *testing.T) {
	...
	if got := GetProvider().Name(); got != "anipub" {
		t.Fatalf("expected anipub provider when allanime is disabled, got %q", got)
	}
}
...
func TestGetProviderFallsBackWhenConfiguredProviderDisabled(t *testing.T) {
	...
	if got := GetProvider().Name(); got != "anipub" {
		t.Fatalf("expected anipub fallback for disabled provider, got %q", got)
	}
}
