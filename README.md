...
> [curd by Wraient](https://github.com/Wraient/curd), rebuilt around provider
> resolution that had stopped finding anime at all, with a new provider stack,
> reworked tracking, and theming that follows the desktop. Original work © its
> contributors; modified since 2026 and released under the same GPL-3.0 licence.
>
...
> the provider, the quality, the audio track and the theme.
>
> **Coming from curd?** Your config, tokens and watch history are carried over
> automatically on first launch. The originals are copied, not moved, so an
> existing curd install keeps working; you can delete them once this looks right.
>
...
> | Finishing an episode could lose it: MPV ran past the end onto a playlist placeholder, otakase chased that as a request for episode 1, and the failed switch had already cleared the progress it needed to record | Running off the end ends playback instead, the playlist follows the audio actually playing, and a switch that fails restores everything it cleared |
...
> - `otakase -provider-status` probes every provider and reports which ones work.
>
> *Playback*
> - **A show carried in only one language just plays.** Reaching the other audio
>   took two menus for a question with one useful answer; otakase now switches and
...
> - `otakase -download` saves episodes instead of streaming them — the most-requested
...
>
> *Interface*
> - **Otakase says when it is starting**, if starting takes more than about a second
...
> `otakase -provider-status` rather than trusting this table:
>
> | Provider | State |
> |---|---|
...
https://matrix.to/#/#otakase:matrix.org
...
> **Note**: `Otakase` requires `mpv`. `rofi` and `ueberzugpp` are optional, for the
> graphical menus and image previews.

> **Installing this fork?** It is a drop-in replacement for the official `otakase`
> package: same binary name, same `~/.config/otakase/otakase.conf`, same AniList and
...
>
> This fork is **not on the AUR**, so `yay -S otakase` installs the *original*
...
```bash
sudo pacman -R otakase && yay -S otakase
```
</details>
...
1. Add otakase as a flake input, for example:
```nix
{
    inputs = {
        nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
        otakase = {
            url = "github:TheXykril/otakase";
...
{inputs, pkgs, ...}: {
  environment.systemPackages = [
    inputs.otakase.packages.${pkgs.system}.default
  ];
}
```
...
- Download and run the [Windows Installer](https://github.com/TheXykril/otakase/releases/latest/download/otakase-windows-installer.exe)

Option 2: Standalone executable
- Download [otakase-windows-x86_64.exe](https://github.com/TheXykril/otakase/releases/latest/download/otakase-windows-x86_64.exe)
</details>
...
```bash
C:\.local\share\otakase
```

Config : 

```bash
C:\Users\USERNAME\AppData\Roaming\Otakase
```
...
Run `otakase` with the following options:
...
> - Rofi `.rasi` themes live in `~/.local/share/otakase/`.
...
- **Add a New Anime**:
  ```bash
  otakase -percentage-to-mark-complete=90
  ```

- **Play with Rofi and Image Preview**:
  ```bash
  otakase -rofi -image-preview
  ```

## Downloading

Otakase can save episodes instead of streaming them. This needs `ffmpeg`.
...
# Download a range
otakase -download -episodes 1-12

# Download somewhere specific
otakase -download -episodes 5 -download-dir ~/Videos/anime
```

You pick the anime the same way as for playback — the whole selection flow is
shared — and then Otakase saves rather than plays. Files land in
`$HOME/Downloads/otakase` by default (set `DownloadDir` in the config), named
...
Otakase colours its menus from your desktop theme where it can.
...
the rofi menus follow it. Switch themes with `omarchy theme set <name>` and Otakase
picks it up on its next run — nothing to configure.

Anywhere else, Otakase uses its own palette.

Set `Theme` in `~/.config/otakase/otakase.conf`:
...
| `builtin` | Always use Otakase's own palette, and stop rewriting the rofi themes |
...
config file is located at ```~/.config/otakase/otakase.conf```

On first start, otakase asks which tracking mode you want to use:
...
Local history stays enabled in all modes. `anilist` means local history + AniList sync, `myanimelist` means local history + MyAnimeList sync, and `anilist + myanimelist` updates both platforms. In dual-sync mode, otakase compares the latest remote update time for each anime and pushes the newest status/progress/category back to the older tracker so both sides converge automatically. Legacy installs are migrated to `anilist` automatically. If you switch trackers later, otakase exposes a **Change Tracker** menu entry and can either merge both lists, replace MyAnimeList with AniList, or replace AniList with MyAnimeList.
...
If the browser reaches the localhost callback page but otakase does not continue automatically, rerun the command and paste the full callback URL when prompted. Otakase now keeps the pending MyAnimeList PKCE state so the login can be completed manually.
...
| `AddMissingOptions`       | Boolean    | `true`, `false`                           | When `true`, on a **version upgrade** only options registered as new for that release are **appended** to `otakase.conf` (not every historical default). Set `false` to opt out. Defaults still apply in memory always. |
| `VimKeys`                 | Boolean    | `true`, `false`                           | When `true`, selection menus use vim motions: `j`/`k`/`h`/`l` (and arrows) to move, `/` or `?` to search. Default: `false` (type-to-filter immediately). Add `VimKeys=true` to `~/.config/otakase/otakase.conf` to enable. |
...
| `MpvEpisodePlaylist`    | Boolean    | `true`, `false`                           | When `true` (default), after playback is stable otakase fills the MPV playlist with episodes (and dub/sub when available) so you can pick from MPV's playlist UI. Access: `MPV` → right-click the prev/next OSC button (or `Ctrl+P`). Selecting a row loads that stream and keeps tracking in sync. |
...
| `StoragePath`             | String     | Any valid path (Environment variables accepted)  | Specifies the directory where Otakase stores its data.                                        |
...
| `Player`                  | String     | any mpv-compatible binary (e.g. `mpv`, `iina`) | Player binary used for playback. If not found, Otakase falls back to `mpv`.                          |
...
| `TrackingRemote`          | Enum       | `none`, `anilist`, `myanimelist`, `anilist+myanimelist` | Selects which remote tracker otakase syncs with.                                           |
...
**Otakase is slow to search.** A provider that fails repeatedly is skipped for five
minutes, and slow providers are abandoned once a faster one has answered, so
this usually resolves itself. `otakase -provider-status` shows the per-provider
timings.
...
peers are found, then it plays and seeks normally. Otakase seeds while you watch,
...
directory removed when otakase exits. Nyaa's feed returns roughly the most recent
...
**Logs.** `~/.local/share/otakase/debug.log`, truncated on each run.
